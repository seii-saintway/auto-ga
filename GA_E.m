function [population, generation, legend, era] = GA_E(f, a, b, delta, stringsAll, stringsCrossed, stringsMutated, bitsAll, bitsCrossed, bitsMutated)
...
while true
    ...
    for i = [I(1: 2: stringsCrossed); I(2: 2: stringsCrossed)]
        if any(i == N2); continue; end;
        rng(RbC);
        for j = randperm(bitsAll, bitsCrossed)
            ...
            intermediation([i(1) i(2)], k) = bitset(intermediation([i(1) i(2)], k), J, bitget(intermediation([i(2) i(1)], k), J));
        end
        RbC = rng;
    end
